...
if (isset($_POST['selected_parcelles']) && isset($user_id) && $erreurs == 0) {
    ...
    if ($erreurs > 0) {
        ...
        $_SESSION['affichage_reservation'] = 
            ...
            header('location: ../user/reservation.php?id_jardin='.$id_jardin);
    } else {
        if(!empty($_POST['selected_parcelles'])) {
            $_SESSION['affichage_reservation'] = 
                ['message' => "<div class='reserv-par'>Les parcelles sélectionnées sont désormais réservées. <a href='dashboard.php'> Aller au Dashboard</a></div>"];
            header('location: ../user/reservation.php?id_jardin='.$id_jardin);
        } else {
            $_SESSION['affichage_reservation'] = 
                ['message' => "<div class='reserv-par-2'>Aucune parcelle sélectionnée.</div>"];
            header('location: ../user/reservation.php?id_jardin='.$id_jardin);
        }
    }
?>
