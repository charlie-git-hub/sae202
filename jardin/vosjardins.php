...
<html lang="en">
  <head>
    ...
    <title>Vos Jardins ! | Potajou</title>
  ...
  ?>
    <header>
      ...
      <div class="pres-p">
      <h1 class="title--01">Résumé de vos Jardins <?php echo $prénom; ?> <span>👋🏻</span></h1>
      <p>Bienvenue sur votre liste de Jardins !
        Ici, vous pouvez apercevoir vos jardins, ainsi que les propositions qui sont vous faîtes !</p>
      </p>
    ...
    if (empty($jardins_confirm)) {
        ...
        foreach ($jardins_grouped_confirm as $jardin) {
            ...
            echo '<img src="../images/uploads_jardin/'.$jardin['img_jardins'].'" alt="Image du jardin" class="jardin-image">'; 
...
?>
      ...
      <h1 class="awn">Réponses positives des propriétaires</h1>
    <?php
    if (empty($jardins_confirm)) {
      echo "<h3 class='sad-face'>😞</h3>";
      echo "<h3 class='reserv'>Vous n'avez aucune réponses confirmé.</h3>";
  } else {
      $jardins_grouped_confirm = [];
      foreach ($jardins_confirm as $confirmation) {
          $id_jardin = $confirmation['id_jardins'];
          if (!isset($jardins_grouped_confirm[$id_jardin])) {
              $jardins_grouped_confirm[$id_jardin] = [
                  ...
                  'proprietaire' => $confirmation['proprietaire'],
                  'parcelles' => [],
                  'date_debut' => $confirmation['date_debut_parcelles'],
                  'date_fin' => $confirmation['date_fin_parcelles'],
                  'taille' => $confirmation['taille_jardins'],
              ];
          }
      $jardins_grouped_confirm[$id_jardin]['parcelles'][] = $confirmation['id_parcelles'] - (25 * ($id_jardin - 1));
  }
  ?>
                    <?php
    foreach ($jardins_grouped_confirm as $jardin_conf) {
        echo '<div class="grid-container">';
        echo '<div class="grid-item"><h1>Nom du Jardin</h1>' . $jardin_conf['nom_jardins'] . '</div>';
        echo '<div class="grid-item align">' . implode('-', $jardin_conf['parcelles']) . '</div>';
        echo '<div class="grid-item align"><h1>Propriétaire</h1>' . $jardin_conf['proprietaire'] . '</div>';
        echo '<div class="grid-item align flex-column"><h1>Durée</h1>';
        echo date("d/m/Y", strtotime($jardin_conf['date_debut']));
        echo '<p>jusqu\'au</p>';
        echo date("d/m/Y", strtotime($jardin_conf['date_fin'])) . '</div>';
        echo '<div class="grid-item align"><h1>Surface</h1>' . ($jardin_conf['taille']/25*count($jardin_conf['parcelles'])) . ' m²</div>';
        echo '</div>';
    }
  }
?>
      <hr class="separatorr-6"></hr>
      <h1 class="t-05">En attente de votre réponse</h1>
      <?php
                ...
                if (empty($jardins_attente)) {
                    echo "<h3 class='t-06'>Vous n'avez aucune parcelle en attente de confirmation.</h3>";
                } else {
                ?>
                <?php
                foreach($jardins_attente as $jardin_attente) {
                  echo '<div class="jardin_attente">';
                  echo '<div class="grid-item">'.$jardin_attente['nom_jardins'].'</div>';
                  echo '<div class="grid-item align">'.$jardin_attente['id_parcelles'] - (25 * ($jardin_attente['id_jardins'] - 1)).'</div>';
                  echo '<div class="grid-item align">'.$jardin_attente['prénom_compte'].'</div>';
                  echo '<div class="grid-item align flex-column">';
                  echo '<div>'.date("d/m/Y", strtotime($jardin_attente['date_debut_parcelles'])).'</div>';
                  echo '<p>jusqu\'au</p>';
                  echo '<div>'.date("d/m/Y", strtotime($jardin_attente['date_fin_parcelles'])).'</div>';
                  echo '</div>';
                  echo '<div class="grid-item align bouton_rep">';
                  echo '<form action="../traitement/reservation_valid.php" method="post">';
                  ...
                  echo '|';
                  echo '<form action="../traitement/reservation_refus.php" method="post">';
                  echo '<input type="hidden" name="nom" value="'.$jardin_attente['prénom_compte'].'">';
                  echo '<input type="hidden" name="id_parcelles" value="'.$jardin_attente['id_parcelles'].'">';
...
H230C221.9,78,215.2,72.8,215.2,64z"
            ...
            <ul>
                ...
                <li><a href="../user/dashboard.php" class="tb-active2">
...
401 -27 24 c-77 66 -870 719 -885 727 -13 7 -43 -13 -140 -94z"/>
                ...
                <li><a href="../user/profil.php" class="tb-active3">
                <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
...
-19 160 0 270 77 441 496 733 932 650z"/>
                ...
                <li><a href="../user/profil.php" class="tb-active-2">
                <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
 width="20.000000pt" height="20.000000pt" viewBox="0 0 500.000000 500.000000"
 preserveAspectRatio="xMidYMid meet">
...
26 59 0 16 -11 42 -26 59 -21 26 -33 31 -69 31 -36 0 -48 -5 -69 -31z"/>
  ...
  <div class="pv-1">
    <h1>Retrouvez vos Jardins/Parcelles ici, <?php echo $prénom; ?>👋🏻</h1>
    </a>
    <h1 class="t-03">Vos Réservations</h1>
    <?php
    if (empty($jardins_confirm)) {
...
?>
<hr class="separatorr-9"></hr>
  </div>
  <div class="bv-p">
    <div class="info-2"> 
      </div>
  </div>
  <h1 class="awn-2">Réponses positives des propriétaires</h1>
  <?php
    if (empty($jardins_confirm)) {
      echo "<h3 class='sad-face'>😞</h3>";
      echo "<h3 class='reserv'>Vous n'avez aucune réservation confirmée.</h3>";
    } else {
      $jardins_grouped_confirm = [];
      foreach ($jardins_confirm as $confirmation) {
        $id_jardin = $confirmation['id_jardins'];
  ...
  ?>  
  <hr class="separatorr-8"></hr>
  <h1>Vos réservation en attente</h1>
  <?php
                $req2 = $mabd->prepare('SELECT jardins.taille_jardins, jardins.id_jardins,jardins.nom_jardins, parcelles.id_parcelles, proprietaire.prénom_compte AS proprietaire, parcelles.date_debut_parcelles, parcelles.date_fin_parcelles FROM parcelles INNER JOIN jardins ON parcelles._id_jardins = jardins.id_jardins INNER JOIN compte AS proprietaire ON jardins._id_compte = proprietaire.id_compte WHERE parcelles._id_compte = '.$_SESSION['id'].' AND parcelles.reservation_parcelles = 1;');
                $req2->execute();
                $jardins_reserv = $req2->fetchAll();

                if (empty($jardins_reserv)) {
                    echo "<h3>Vous n'avez aucune réservation en attente.</h3>";
                } else {
                    $jardins_grouped = [];
                    foreach ($jardins_reserv as $reservation) {
                        $id_jardin = $reservation['id_jardins'];
                        if (!isset($jardins_grouped[$id_jardin])) {
                            $jardins_grouped[$id_jardin] = [
                                'nom_jardins' => $reservation['nom_jardins'],
                                'proprietaire' => $reservation['proprietaire'],
                                'parcelles' => [],
                                'date_debut' => $reservation['date_debut_parcelles'],
                                'date_fin' => $reservation['date_fin_parcelles'],
                                'taille' => $reservation['taille_jardins'],
                            ];
                        }
                    $jardins_grouped[$id_jardin]['parcelles'][] = $reservation['id_parcelles'] - (25 * ($id_jardin - 1));
                    }
                ?>
                <table class="table-jardin">
                    ...
                    foreach ($jardins_grouped as $jardin) : ?>
                      <tr>
                          <td><?= $jardin['nom_jardins'] ?></td>
                          <td class="align"><?= implode('-', $jardin['parcelles']) ?></td>
                          <td class="align"><?= $jardin['proprietaire'] ?></td>
                          <td class="align">
                              <div class="date">
                                  <?= date("d/m/Y", strtotime($jardin['date_debut'])) ?> jusqu'au <br>
                                  <?= date("d/m/Y", strtotime($jardin['date_fin'])) ?>
                              </div>
                          </td>
                          <td class="align"><?= ($jardin['taille'] / 25 * count($jardin['parcelles'])) ?> m²</td>
                      </tr>
                  <?php endforeach;} ?>
              </tbody>
          </table>
                ...
                <h2 class="attente-vous">En attente de votre confirmation :</h2>
                <?php
                $req3 = $mabd->prepare('SELECT * FROM parcelles INNER JOIN jardins ON parcelles._id_jardins = jardins.id_jardins INNER JOIN compte ON parcelles._id_compte=compte.id_compte WHERE `reservation_parcelles` = 1 AND jardins._id_compte = '.$_SESSION['id'].';');
                $req3->execute();
                $jardins_attente = $req3->fetchAll();
...
if (empty($jardins_attente)) {
    ...
    echo '<table style="width: 100%; border-collapse: collapse; border: 1px solid #ddd; font-family: Arial, sans-serif;">';
    echo '<thead>';
    echo '<tr>';
    echo '<th style="text-align: center; border: 1px solid #ddd; padding: 8px;">Nom du jardin</th>';
    echo '<th style="text-align: center; border: 1px solid #ddd; padding: 8px;">Numéro de parcelle</th>';
    echo '<th style="text-align: center; border: 1px solid #ddd; padding: 8px;">Demande de l\'utilisateur</th>';
    echo '<th style="text-align: center; border: 1px solid #ddd; padding: 8px;">Date début/fin</th>';
    echo '<th style="text-align: center; border: 1px solid #ddd; padding: 8px;">Confirmation ?</th>';
    echo '</tr>';
    echo '</thead>';
    echo '<tbody>';
    ...
    foreach($jardins_attente as $jardin_attente) {
        ...
        echo '<td style="text-align: center; border: 1px solid #ddd; padding: 8px;">'.$jardin_attente['nom_jardins'].'</td>';
        echo '<td style="text-align: center; border: 1px solid #ddd; padding: 8px;">'.($jardin_attente['id_parcelles'] - (25 * ($jardin_attente['id_jardins'] - 1))).'</td>';
        echo '<td style="text-align: center; border: 1px solid #ddd; padding: 8px;">'.$jardin_attente['prénom_compte'].'</td>';
        echo '<td style="text-align: center; border: 1px solid #ddd; padding: 8px;">';
        echo '<span style="display: flex; flex-direction: column; align-items: center;">';
        echo date("d/m/Y", strtotime($jardin_attente['date_debut_parcelles']));
        echo '<p>jusqu\'au</p>';
        echo date("d/m/Y", strtotime($jardin_attente['date_fin_parcelles']));
        echo '</span>';
        echo '</td>';
        echo '<td style="text-align: center; border: 1px solid #ddd; padding: 8px;">';
        ...
        echo '<input type="hidden" name="nom" value="'.$jardin_attente['prénom_compte'].'">';
        echo '<input type="hidden" name="id_parcelles" value="'.$jardin_attente['id_parcelles'].'">';
        echo '<button type="submit" style="width: 30px; height: 30px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; background-color: green; color: white;">O</button>';
        echo '</form>';
        echo '/';
        echo '<form action="../traitement/reservation_refus.php" method="post">';
        echo '<input type="hidden" name="nom" value="'.$jardin_attente['prénom_compte'].'">';
        echo '<input type="hidden" name="id_parcelles" value="'.$jardin_attente['id_parcelles'].'">';
        echo '<button type="submit" style="width: 30px; height: 30px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; background-color: red; color: white;">X</button>';
        echo '</form>';
        echo '</div>';
        echo '</td>';
        echo '</tr>';
