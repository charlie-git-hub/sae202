...
<head>
  ...
  <title>Votre Profil | Potajou</title>
...
<?php
    ...
    if (isset($_SESSION['prénom'])) {
      ...
      $id = $_SESSION['id'];
      $req = $mabd->prepare('SELECT * from compte');
      $req->execute();
      $compte = $req->fetchAll();
      foreach($compte as $info) {
        $pdp = $info['img_compte'];
        $mail = $info['mail_compte'];
      }
  }

  $req5 = $mabd->prepare('SELECT jardins.taille_jardins, jardins.id_jardins,jardins.nom_jardins, parcelles.id_parcelles, proprietaire.prénom_compte AS proprietaire, parcelles.date_debut_parcelles, parcelles.date_fin_parcelles FROM parcelles INNER JOIN jardins ON parcelles._id_jardins = jardins.id_jardins INNER JOIN compte AS proprietaire ON jardins._id_compte = proprietaire.id_compte WHERE parcelles._id_compte = '.$_SESSION['id'].' AND parcelles.reservation_parcelles = 2;');
                $req5->execute();
                $jardins_confirm = $req5->fetchAll();
?>
...
<section class="total">
      <nav>
        ...
        <?php
            echo '<img class="profil-img" src="../images/uploads_pdp/'.$pdp.'" alt="Photo de profil">';
          ?>
          <?php echo $prénom; ?>
        </div>
        </a>
        <button class="cta">
          <span class="hover-underline-animation">DECONNEXION</span>
          <svg
            id="arrow-horizontal"
      ...
      <div class="pres-p2">
      <?php
            echo '<img class="profil-img2" src="../images/uploads_pdp/'.$pdp.'" alt="Photo de profil">';
          ?>
      <h1>Ceci est votre profil, <?php echo $prénom; ?> 👋🏻</h1>
      <div class="info">
      <h4>Nom d'utilisateur: <?php echo $prénom; ?></h4>
      <h4>Votre E-mail: <?php echo $mail; ?></h4>
      <div class="btn-c">
      <a href="../traitement/update_mdp.php"><button type="submit" class="form-btn-2">Modifier votre nom</button></a>
      <a href="../traitement/update_mdp.php"><button type="submit" class="form-btn-2">Modifier le mot de passe</button></a>
      <a href="../traitement/update_mail.php"><button type="submit" class="form-btn-3">Modifier l'email</button></a>
      </div>
</div>
    </div>
    ...
    <header>
      ...
      <div class="hamburger">
        <div class="parent">
          ...
          <div class="div2">
            <a href="dashboard.php">VOS RESERVATIONS</a>
            <svg width="65" height="50" viewBox="0 0 100 100">
              <g transform="rotate(-45 50 50)">
                <!-- Tige de la flèche -->
                <line
...
401 -27 24 c-77 66 -870 719 -885 727 -13 7 -43 -13 -140 -94z"/>
                ...
                <li><a href="../user/dashboard.php" class="tb-active-2">
                <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
...
26 59 0 16 -11 42 -26 59 -21 26 -33 31 -69 31 -36 0 -48 -5 -69 -31z"/>
  ...
  <div class="bv-p">
    <div class="info-2">
      <h4>Nom d'utilisateur: <?php echo $prénom; ?></h4>
      <h4>Votre E-mail: <?php echo $mail; ?></h4>
      <a href="../traitement/update_nom.php"><button type="submit" class="button-22">Modifier votre nom</button></a>
      <a href="../traitement/update_mdp.php"><button class="button-22">Modifier le mot de passe</button></a>
      <a href="../traitement/update_mail.php"><button class="button-22">Modifier l'e-mail</button></a>
    </div>
  </div>
</div>
